...
# Apply progress — PR4a1 (additive relationship-integrity trigger migration + DB-boundary/round-trip tests)

## Instrumentation consumed

- Status: `changeName` `represented-person-account-flow`; `artifactStore` `openspec`; `applyState` ready (apply dependency ready); `actionContext.mode` `repo-local`; allowed edit root = this worktree (`pi-1137-pr4a1`); `actionContext.warnings` none. Authoritative inputs read before editing: `tasks.md`, `design.md` (PostgreSQL safeguards, §Constraints and triggers), all six delta specs (`representation-lifecycle/spec.md` line-by-line for the database-boundary scenarios), `proposal.md`, `apply-progress.md` (merged, not overwritten), `openspec/config.yaml` (`strict_tdd: true`). The native attempt token was consumed as an opaque correlation id only and is **not** persisted in this artifact.
- Scope honored: **only** the additive Alembic relationship/phone trigger migration and its direct-SQL/round-trip migration tests — no shared Python validator, router/DTO, reassignment, safe stop, model/domain change, or `tasks.md` edit. Parent-authorized PR 4 split: **PR4a0** (legacy adult-link fixtures, committed as `32358b7`) + **PR4a1** (this slice) + **PR4b** (shared `RelacionRepresentacionServicio` validator/commands, router cutover, reassignment, safe stop; still unimplemented here).
- Read-only oracle `/home/alejo/devwork/.projects/apps/cata_club-worktrees/pi-1137-pr4a` was used strictly read-only and not edited; all three authored files are byte-identical to the oracle's newest corrected implementation (`diff -q` verified, re-confirmed during this compaction).
- Checkbox policy: `tasks.md` was **not modified** by PR4a1, so **0 task rows are reported complete by this slice**. PR4a1 supplies only the *database half* of row 1, whose "shared validator" half is PR4b; marking row 1 complete now would be a false completion, and the parent instruction forbids it. Therefore no persisted `- [ ]` contradicts a claimed completion. The three rows remain, verbatim:
```text
- [ ] Shared validator owns self/cycle/age/phone/reachability invariants with database defense.
- [ ] Atomic reassignment with documented lock order, stale conflict, audit, epoch revocation, and post-commit notification.
- [ ] Non-disclosing safe stop replaces self-service linking.
...
|---|---|---|---|---|---|---|---|
| DB-boundary candle | `backend/tests/test_representacion_triggers.py` | PostgreSQL direct-SQL / trigger integration | `69 passed, 4 warnings` pre-change baseline | ✅ Written first with the migration absent: `15 failed, 8 passed` (`DID NOT RAISE DBAPIError` at `_ejecutar_y_preservar`, `assert None is False` for the absent CHECK) | ✅ Passed: `23 passed, 1 warning` after adding the migration | ✅ 2-file focused `23 passed`; 3-file `35 passed`; 11-file adjacent `136 passed`; independent CLI round trip + 15-case direct-SQL matrix | ✅ ruff `All checks passed!`; AST parse OK; zero whitespace findings |
| Migration round trip | `backend/tests/test_representados_alcanzables.py` | PostgreSQL migration harness (`arnes_migracion`, ephemeral DB) | same baseline | ✅ Written first: `test_el_round_trip_downgrade_upgrade_restaura_el_candado` and `test_los_triggers_documentados_son_los_instalados` both failed (`revision_actual()` was `h1140rep_auditoria`, not `i1141relinteg`) | ✅ Passed with the migration present | ✅ Also proven outside pytest by real `alembic` CLI on a scratch database (evidence 1) | ✅ same as above |

### Exact RED / GREEN / TRIANGULATE / REFACTOR commands and results

- Tenancy proof first (no competing process/service): `docker ps --filter publish=5436` → **exactly one** `pi-1137-pr4a-db-test-1` (`127.0.0.1:5436->5432/tcp`, healthy, Up 35 min); `pgrep -af 'pytest|alembic'` → no pytest/alembic process; `select count(*) from pg_stat_activity where datname='cataclub_test' and pid<>pg_backend_pid()` → **0**; baseline schema state `alembic_version = h1140rep_auditoria` with only the legacy `trg_persona_representante_alcanzable`. The existing container was **not** stopped, recreated, or re-projected onto a second project name (that would have double-bound :5436).
- Safety net: `uv run pytest tests/test_migracion_representados_alcanzables.py tests/test_representante_no_deja_menores_huerfanos.py tests/test_vinculacion_representante.py tests/test_migracion_representante_auditoria.py tests/test_independencia_representada.py -q -p no:randomly` → **69 passed, 4 warnings in 30.61s**.
- RED: `uv run pytest tests/test_representacion_triggers.py tests/test_representados_alcanzables.py -q -p no:randomly` (migration file absent; both test files already authored) → **15 failed, 8 passed, 1 warning in 3.09s**. Representative failure lines: `tests/test_representacion_triggers.py:82: Failed: DID NOT RAISE DBAPIError` (relation trigger not installed; legacy `g1139` trigger rejected the inactive-minor unlink case with `persona_id=2 es menor de edad (6 años) y no puede quedar sin representante`) and `tests/test_representacion_triggers.py:97: assert None is False` (`ck_persona_representante_no_autoreferencia` absent from `pg_constraint`).
- GREEN: the same RED command with `backend/alembic/versions/i1141relinteg_triggers_de_relacion.py` added → **23 passed, 1 warning in 3.24s**.
- TRIANGULATE (focused 3 files): `uv run pytest tests/test_representacion_triggers.py tests/test_representados_alcanzables.py tests/test_migracion_representados_alcanzables.py -q -p no:randomly` → **35 passed, 1 warning in 14.75s**. TRIANGULATE (adjacent 11-file relationship suite, at head): `uv run pytest tests/test_representacion_triggers.py tests/test_representados_alcanzables.py tests/test_migracion_representados_alcanzables.py tests/test_representante_no_deja_menores_huerfanos.py tests/test_vinculacion_representante.py tests/test_migracion_representante_auditoria.py tests/test_independencia_representada.py tests/test_baja_logica_persona.py tests/test_beneficio_autoservicio.py tests/test_inventario_anomalias_membresias.py tests/test_notificaciones_paginacion.py -q -p no:randomly` → **136 passed, 7 warnings in 37.27s** (the PR4a0 fixture corrections keep every adjacent flow green under the new candle). TRIANGULATE (root guards): `make test-root` → **582 passed, 1 skipped in 22.06s** (includes `tests/test_alembic_cabeza_unica.py`, the single-head guard for the new revision).
- REFACTOR: `uv run ruff check` on the three new files → `All checks passed!`; `ast.parse` on all three → `AST_OK`; `git diff --check` → clean; `git diff --no-index --check /dev/null <file>` ×3 → no whitespace findings.

## Independent triangulation evidence (outside the pytest harness)

1. **Real `alembic` CLI round trip on scratch database `pr4a1_roundtrip`** (dropped afterwards), not the test harness path: empty → `upgrade head` = `i1141relinteg (head)` with `persona` triggers `trg_relacion_representacion_valida` + `trg_telefono_representante_con_menores`; `downgrade h1140rep_auditoria` = revision `h1140rep_auditoria` with the legacy `trg_persona_representante_alcanzable` restored and the new triggers gone; `upgrade head` again = both candles back and `ck_persona_representante_no_autoreferencia` re-added with `convalidated = f` (still `NOT VALID`, as designed). This proves **head → h1140rep_auditoria → head** independently of the pytest round-trip test.
2. **15-case direct-SQL diagnostic matrix** on a scratch database at head, with the exact Postgres error text captured: raw `INSERT` of an adult **with** `representante_id` → `persona_id=900001 es mayor de edad y no puede vincularse o re-enlazarse a un representante`, and `SELECT count(*) FROM persona WHERE id=900001` → `0` (graph unchanged); `UPDATE` re-link of an aged-in-place adult → same adult rejection, and `UPDATE` first link of an adult (`representante_id` previously NULL) → same rejection; minor link `INSERT`, aging in place, and editing an unrelated field → **admitted** (the `IS NOT DISTINCT FROM` short-circuit keeps legacy aged-in-place adults editable and able to unlink); self-reference `UPDATE representante_id = id` → `no puede ser su propio representante`; 2-node and 3-node minor-only cycles → `formaría un ciclo de representación`, graph intact `[(1, None), (70, None), (71, 70), (72, 71)]`; destination with a deactivated account → `su cuenta está desactivada`; destination without a valid current phone (seeded by raw SQL to bypass ORM `@validates`) → `el destino no tiene un teléfono válido actual`; representative with an **active** minor dependent changing the phone to `123` → `representa a un menor activo y no puede dejar un teléfono sin forma válida`, while with only an aged-in-place adult dependent → **admitted**; clearing the link of an **active** minor → rejected, of an **inactive** minor or of an adult → **admitted**.
3. Two first-pass diagnostics were correctly re-run because the seed collided with a *different* earlier guard (an explicit-id collision made the first raw `INSERT` hit the self-reference branch; an adult re-link hit the age branch before the recursive CTE could run). The re-run with unambiguous seeds isolated the intended guards, so the age and cycle defenses are proven by their own error messages rather than by a neighbouring guard.

## Files changed (all three NEW; zero deletions; no other slice tracked-modified)

- `backend/alembic/versions/i1141relinteg_triggers_de_relacion.py` — 303 lines. `revision = i1141relinteg`, `down_revision = h1140rep_auditoria`. Adds `ck_persona_representante_no_autoreferencia` (`NOT VALID`), replaces the `g1139repmenor` persona trigger with `BEFORE INSERT OR UPDATE OF representante_id` `trg_relacion_representacion_valida` (self-link rejection, adult `INSERT` **and** `UPDATE` re-link rejection, `pg_advisory_xact_lock(7113911370001)` graph mutex, destination `usuario` row lock + reachability/phone check, recursive-CTE cycle rejection, active-minor-only unlink protection), and adds `BEFORE UPDATE OF telefono` `trg_telefono_representante_con_menores` (canonical `^(09[0-9]{8}|0[0-9]{8})$` parity with `_RE_TELEFONO_FORMA`). `downgrade()` drops both new candles, drops the CHECK without validating it, and restores the exact `g1139repmenor` trigger SQL.
- `backend/tests/test_representacion_triggers.py` — 422 lines, 15 tests (parametrized → 23 cases): the NOT VALID CHECK, raw-SQL self-reference (insert/update), 2- and 3-node cycles, adult re-link, adult raw `INSERT`, aged-in-place edit/unlink, active vs. inactive minor unlink, destination with inactive account, invalid destination phone (ORM and raw-SQL seeds) plus the passing link after correction, phone safeguard with and without active minors, same-value phone rewrite, out-of-scope writes, and the documented graph mutex key (`pg_advisory_xact_lock` held by a second transaction + `lock_timeout`).
- `backend/tests/test_representados_alcanzables.py` — 204 lines, 4 tests: trigger doc/database parity (the two new candles installed, legacy `g1139` persona trigger gone, `usuario` trigger alive), the #1139 active-minor account-deactivation regression, the aged-out minor releasing that deactivation, and the `head → h1140rep_auditoria → head` round trip with contract-level assertions (legacy trigger admits the raw adult `INSERT` it can no longer block; the restored candle rejects it again and leaves no row).
- `openspec/changes/represented-person-account-flow/apply-progress.md` — this cumulative PR4a1 section only (`tasks.md` untouched; `proposal.md`, `design.md`, and all `specs/**/spec.md` untouched; `verify-report.md` untouched).

## Workload, PR boundary, rollback, deviations, cleanup

- Authored implementation count: **929 additions + 0 deletions = 929 changed lines** (303 + 422 + 204) — **under the 1,000 hard stop**; 29 lines above the 600–900 target, the same accounting the oracle recorded, and the parent explicitly bounded this slice by the 1,000 stop. SDD bookkeeping (this section) is excluded from the implementation count per the established accounting.
- Boundary: `main → tracker #1164 → … → PR 3 (d63e10d) → PR4a0 (32358b7) → 📍 PR4a1`; PR4b (shared validator/commands, router cutover, reassignment, safe stop) is the next slice and remains unimplemented here.
- Rollback boundary: delete the three new files; nothing else exists to revert. No model, domain, service, router, DTO, frontend, or other test file was touched, so rollback removes no unrelated work. The migration's own `downgrade()` restores the exact `g1139repmenor` trigger SQL, and that restoration is proven by both the pytest round-trip test and the independent CLI round trip.
- Deviations from the design/oracle: **none** — the three files are byte-identical to the read-only oracle. The only deliberate difference from the oracle's artifact set is that the PR4a0 fixture corrections are already committed at parent `32358b7`, so they are not re-authored here, and this slice does not update the oracle's `verify-report.md`.
- Not run here (recorded, **not** claimed): no full `make pre-pr LANE=backend|frontend` lane, no QA runtime, no commit, no push, no PR, and no remote CI gate. The strongest local evidence for this slice is the focused PostgreSQL suites above plus `make test-root`. Runtime: N/A for this slice — there is no endpoint or UI surface; the database boundary itself is what is exercised (recorded here for the PR body per `tasks.md`).
- Process cleanup: both scratch databases (`pr4a1_roundtrip`, `pr4a1_roundtrip2`) were dropped; `pg_database` shows only `cataclub_test` and `postgres`; `cataclub_test` has 0 external connections; the pre-existing healthy `pi-1137-pr4a-db-test-1` (:5436) container was left untouched.

## Evidence fingerprints (settle-ready)

- Base revision (`HEAD`): `32358b78a814dc34bc94ddc4ec52df01895193c8` (tree `3aab8d749d63860f853acffb0946330c1cf4ff11`). `backend/alembic/versions/i1141relinteg_triggers_de_relacion.py` → `sha256:cdb6a5da7ece07140a8b95cb06223b58d6bd58ad7547d92a4c991bb1b5f421b3` (11892 bytes); `backend/tests/test_representacion_triggers.py` → `sha256:181675fe513ba723ab3ceef686fe481fca2d1135566bf51f3d294ee6d4845ab8` (19538 bytes); `backend/tests/test_representados_alcanzables.py` → `sha256:79b08b637be4a72668ec2ad89ae4382c37261ebfce4de3afe8a10118b2296164` (8194 bytes).
- Slice fingerprint (path + per-file sha256 manifest, canonical order): `sha256:a102648b623d36e26c3221434f6f1af350bd37a64f3b0121c2d402bd57e50bbb`.
- Compacted PR4a1 evidence revision: `sha256:227b6375cb508f7eaf662c174ecedb2fd3fdad3a4fa5f2d0fb48339a50c1ccac` — sha256 over this section's bytes from its `# Apply progress — PR4a1` heading through the end of the preceding line (this line excluded; LF endings).
