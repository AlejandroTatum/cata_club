/**
 * `/forgot-password` — regression for #1209: `document.title` measured
 ...
 *
 * `title.absolute`, not the admin template: a visitor recovering a
 * forgotten password is not staff.
 */

import { describe, it, expect } from "vitest";
import { metadata } from "@/app/forgot-password/layout";

describe("forgot-password layout metadata", () => {
  it("sets an absolute, non-admin title naming the club", () => {
    expect(metadata.title).toEqual({ absolute: "Recuperar contraseña — Cata Club" });
  });
});
