...
describe("DELETE /api/sponsors/[id]", () => {
  ...
  it("rejects invalid identifiers", async () => {
    const response = await DELETE(request(`${ACCESS_TOKEN_COOKIE}=${token}`), { params: Promise.resolve({ id: "x" }) });
  ...
  it("deletes the hosted logo through the backend", async () => {
    ...
    const response = await DELETE(request(`${ACCESS_TOKEN_COOKIE}=${token}`), { params: Promise.resolve({ id: "1" }) });
