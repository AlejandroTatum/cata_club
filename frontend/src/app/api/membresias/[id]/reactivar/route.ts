...
export async function POST(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  const result = await backendFetchAuthed(request, `/membresias/${membresiaId}/reactivar`, {
    method: "POST",
  ...
  if (!result.ok) {
    return NextResponse.json({ message: "No se pudo reactivar la membresía." }, { status: result.status });
  }
  if (!result.response.ok) {
    return passthroughBackendError(result.response, "No se pudo reactivar la membresía.");
  }
