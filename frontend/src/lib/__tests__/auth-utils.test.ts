...
describe("getNavLinksForRole", () => {
  ...
  // The age flag is scoped to "estudiante" only — passing it for
  // "representante" must not add the link, since that access (a guardian
  // managing a DEPENDENT's record) is a separate, unrelated grant.
  it("ignores the adult flag for representante", () => {
    const links = getNavLinksForRole("representante", true);
    expect(links).toHaveLength(4);
    expect(links.some((link) => link.href === "/student/medical-record")).toBe(false);
  });
