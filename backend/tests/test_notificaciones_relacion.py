"""Notificación post-commit de la reasignación de representación (#1133).
...
sin revertir ni invalidar el vínculo ya comiteado. No se agrega ningún
outbox, solicitud ni estado de aprobación de relación.
"""
...
def _representante(db_session, seed: int) -> tuple[Persona, Usuario]:
    ...
    usuario = Usuario(
        ...
        correo_verificado=True, roles=[Rol(tipo_rol=TipoRol.REPRESENTANTE, descripcion="Representante")],
    )
...
def test_fallo_de_notificacion_post_commit_no_revierte_el_exito(db_session):
    """El canal se cae DESPUÉS del commit: la reasignación ya está comiteada
    y el comando la devuelve igual -- el fallo no propaga."""
