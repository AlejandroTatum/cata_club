...
def test_provincia_servicio_contar_provincias_respeta_el_filtro_de_pais(db_session):
    pais = _crear_pais(db_session, "Ecuador")
    ...
    assert servicio.contar_provincias(pais_id=pais.id) == 2
    assert servicio.contar_provincias(pais_id=otro_pais.id) == 1
...
def test_canton_servicio_contar_cantones_respeta_el_filtro_de_provincia(db_session):
    pais = _crear_pais(db_session, "Ecuador")
    provincia = _crear_provincia(db_session, "Pichincha", pais.id)
    ...
    assert servicio.contar_cantones(provincia_id=provincia.id) == 2
    assert servicio.contar_cantones(provincia_id=otra_provincia.id) == 1
