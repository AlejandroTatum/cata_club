...
4. After local verification, present the implementation evidence for the
   reviewable cut to the user — changed paths with a diff summary and the
   result of every local check — and stop for explicit approval. Only that
   explicit approval per reviewable cut (each candidate PR) authorizes the
   delivery steps: commit, push, opening the PR, and, when applicable,
   enabling auto-merge. Implementing and verifying locally never authorize
   delivery on their own.
5. Treat local verification and remote CI monitoring as separate tasks. Local
...
6. Never blindly rerun a failed test or CI check. First classify it as
   **deterministic**, **transient**, or **inherited stacked**, record the
   supporting evidence, and then choose the next action.
7. Record every CI gate skipped locally and why. The pre-PR lanes are predictive
   checks, not a claim of complete GitHub Actions parity.
8. Backend tests use the single-tenant PostgreSQL `db-test` service on port
