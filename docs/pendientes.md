...
| Los errores al usuario salían en inglés cuando el backend peor se portaba | #153 y #155. El default del cliente pasó de `Request failed with status N` a `GENERIC_FAILURE` en español (`frontend/src/services/api.ts:340`; el inglés sobrevive solo en el comentario que documenta la historia, `:336`), y los 27 sitios que renderizaban `err.message` crudo pasaron por `toUserMessage`. Las dos observaciones del traductor que esta fila dejaba abiertas —`GENERIC_FAILURE` cruzando la compuerta 2 y el encabezado ya falso— se cerraron por separado, dos filas más abajo | `frontend/src/lib/__tests__/error-message.test.ts` y el guardián que pedía este ítem, `frontend/src/lib/__tests__/error-message-usage.test.ts` — «only the translator reads an error's message» · 5/5. Corridos el 6 de agosto de 2026 sobre `fa13172` |
| `GENERIC_FAILURE` —el placeholder del propio cliente para «el cuerpo no traía nada usable»— pasaba las dos compuertas y le ganaba al fallback del llamador en 400/409/422 | Issue [#194](https://github.com/AlejandroTatum/cata_club/issues/194): `toUserMessage` agrega un chequeo de identidad contra `GENERIC_FAILURE` antes de devolver el `detail` de un status de `INPUT_STATUSES` — el texto es evidencia de que el cliente no tenía nada que decir, no de que el servidor escribió una oración para una persona; se deja como chequeo de identidad puntual y no como regla léxica en `isUserFacingText`, para no rechazar también copy real del backend que se le parezca. De paso, el segundo genérico de la misma familia fuera del traductor (`services/api.ts`, «No se pudo generar el PDF.») pasa a reusar la misma constante `GENERIC_FAILURE` en vez de su propio texto — llegaba a `toUserMessage` por el mismo camino (`reports/page.tsx` lo captura con su propio fallback, «No se pudo generar el PDF del reporte.») y sufría el mismo defecto | `frontend/src/lib/__tests__/error-message.test.ts` — «toUserMessage — the client's own placeholder is not a server sentence» (2 tests) y «does not by itself reject the client's own generic placeholder», rojo antes (`expected 'No se pudo completar la operación.' to be 'No se pudo guardar el descuento.'`) verde después · 25/25 el archivo completo. Suite completa `npx vitest run` → 161 archivos, 2439/2439; `npm run type-check` y `npm run lint` sin errores |
| El encabezado de `error-message.ts` afirmaba «It is not wired up yet» cuando el cableado a `toUserMessage` ya había pasado, en #153 y #155 | Issue [#194](https://github.com/AlejandroTatum/cata_club/issues/194): se borra el párrafo que describía el módulo como no cableado y se reemplaza por el estado real, con la referencia a #153/#155 y al guardián que ya lo sostenía. No hay candado nuevo para el cierre en sí —es un cambio de tiempo verbal en un comentario, no de comportamiento— pero el estado que ahora describe ya estaba sostenido | `frontend/src/lib/__tests__/error-message-usage.test.ts` — «only the translator reads an error's message» · 5/5 (el mismo guardián de la fila de arriba, que ya afirmaba lo que el comentario negaba). Re-verificado en esta tanda: `rg -c 'toUserMessage\(' frontend/src --glob '!**/__tests__/**'` da 17 archivos/30 coincidencias, pero una de ellas es la propia declaración `export function toUserMessage(...)` en `error-message.ts`, no una llamada; excluyendo ese archivo (`--glob '!**/error-message.ts'`) quedan 16 archivos y 29 llamadas reales. `rg -n 'err(or)?\.message' --glob '*.tsx' --glob '!**/__tests__/**' frontend/src` → sin resultados, igual que antes |
| El mensaje del 408 no puede salir nunca, y en su lugar el usuario leía que la operación «se canceló» | Issue [#190](https://github.com/AlejandroTatum/cata_club/issues/190): `services/api.ts` gana `ApiTimeoutError` (nombre `TimeoutError`, sin `status`) — el `setTimeout` de `DEFAULT_TIMEOUT_MS` (`:231`) fija un flag `timedOut` y un `catch` nuevo (antes solo había `try/finally`) relanza el abort propio como `ApiTimeoutError`; un `AbortSignal` que trae el propio llamador sigue saliendo como `AbortError` sin tocar. En `error-message.ts`, `STATUS_MESSAGES` pierde `408` (nadie lo emite), y de paso `413`/`415`: eran el mismo defecto — un archivo mal formado o demasiado grande lo rechaza `AuthServicio.actualizar_foto_perfil` como `OperacionInvalida`, un **400** cuyo `detail` en español ya pasa las dos compuertas (`backend/app/servicios_negocio/auth_servicio.py:157-169`); la promesa por esos dos status tampoco la producía nadie. Nace `TIMED_OUT` («La operación tardó demasiado. Intente nuevamente.») y `toUserMessage` la devuelve para `name === "TimeoutError"`, antes de la rama `AbortError`. **Lo que el ítem no sabía:** el BFF aborta a los mismos 10 s que el cliente (`frontend/src/lib/server/bff-helpers.ts:19` vs `frontend/src/services/api.ts:231`, ambos `10_000`), así que en la carrera normal gana el abort del cliente — el mensaje falso no era un caso raro, era el camino común de cualquier servidor lento | `frontend/src/lib/__tests__/error-message.test.ts` — «does not call a timeout a cancellation», «carries a producer for every message it can show», «leaves an unsendable status to the caller's own sentence» · `frontend/src/services/__tests__/api.test.ts` — «reports its own timeout as a timeout, not as a cancellation». Corridos con `cd frontend && npx vitest run src/lib/__tests__/error-message.test.ts src/services/__tests__/api.test.ts` → 2 archivos, 72/72 |
| Un fallo de red al consultar el horario se imprimía en el carnet igual que «el club no le asignó horario» | Issue [#192](https://github.com/AlejandroTatum/cata_club/issues/192): la fila «Franja» del carnet ya no depende de una única rama `horariosState.status === "ready"`. Ahora nombra los tres estados: `loading` → «Consultando…», `error` → «No se pudo consultar» (mismo vocabulario que ya usaba el panel «Próximos entrenamientos» para el mismo `horariosState`, así la pantalla no dice dos cosas distintas del mismo fallo), y solo se omite la fila cuando `status === "ready"` y `describeAssignedWindows` devuelve `null` — el único caso real de «el club no asignó horario». `frontend/src/app/student/page.tsx:112-126`. **Lo que el ítem no sabía:** existe un colapso análogo en la fila «Cobertura hasta» del mismo carnet — `coverageEnd` (`page.tsx:553-556`) también resuelve a `null` para `pagosState` en `loading` y en `error`, indistinguible de «nunca hubo un pago aprobado». No se corrigió acá: a diferencia de `horariosState`, ese `coverageEnd` alimenta un segundo consumidor (`describePaymentSituation`, el texto del `PaymentBand`) con su propia lógica de estados, así que el fix correcto es un cambio de contrato con su propio ciclo de TDD, no un añadido a este. Queda para un ítem propio | `frontend/src/app/student/__tests__/StudentPage.test.tsx` — «StudentPage — the carnet's franja agrees with the assigned schedule» › «marks the franja as unavailable on a lookup failure instead of reading like no schedule was assigned». Rojo antes (`Unable to find an element with the text: Franja` — el fallo de red omitía la fila igual que la ausencia real de horario), verde después. Corridos el 9 de agosto de 2026: `cd frontend && npx vitest run src/app/student` → 13 archivos, 309/309; suite completa `npx vitest run` → 161 archivos, 2436/2436; `npm run type-check` y `npm run lint` sin errores |
| `downloadBlob` no tenía timeout ni cancelación, y el encabezado del cliente prometía que sí | Issue [#196](https://github.com/AlejandroTatum/cata_club/issues/196): `downloadBlob` (`frontend/src/services/api.ts:861`) deja de llamar a `fetch(endpoint)` pelado y arma su propio `AbortController` con un `setTimeout`, con el mismo mecanismo `timedOut` que `request()` usa para relanzar el abort propio como `ApiTimeoutError` (nunca como `AbortError`) — así `toUserMessage` sigue leyendo «tardó demasiado», no «se canceló». El número no es el default: nace `PDF_DOWNLOAD_TIMEOUT_MS = 30_000` (`api.ts:847`), justificado en el comentario que lo declara. Por qué 30 s y no los 10 s de `DEFAULT_TIMEOUT_MS`: `proxyBackendPdfGet` (`frontend/src/lib/server/backend-client.ts:167`) no tiene cota propia — pasa por el mismo `backendFetch` que usa cualquier llamada autenticada, y ESE `backendFetch` (`frontend/src/lib/server/auth.ts`) ya aborta solo a los 10 s (`BACKEND_TIMEOUT_MS`), un número pensado para un round-trip de auth, no para renderizar un reporte. Si el cliente se quedara en 10 s empataría esa carrera contra el propio BFF y normalmente la ganaría, convirtiendo lo que podía ser un 503 real y nombrable en un abort de cliente sin más historia. 30 s —el mismo margen que ya usa `FOTO_PERFIL_UPLOAD_TIMEOUT_MS` para otra transferencia lenta— dejar ganar al abort del BFF cuando es él quien se rinde. **Lo que el ítem no sabía:** «el BFF no tiene timeout» no era exacto — sí lo tiene, heredado de `backendFetch`, solo que no es un timeout pensado para PDF; que ese piso de 10 s en el lado del backend/BFF siga siendo corto para una generación de PDF real queda fuera de este cambio (no toca backend) y se deja como seguimiento. El encabezado del módulo (`api.ts:12-19`) pasa a describir la excepción real: todo lo que pasa por `request()` hereda el default de 10 s; `downloadBlob` es la única función que lo bypassea y fija su propia cota más larga. **La primera versión de este fix acotaba solo hasta los headers y la revisión la frenó:** las cuatro lentes coincidieron en que `clearTimeout` vivía en un `finally` colgado únicamente del `await fetch(...)`, que por spec resuelve cuando llegan los **headers** — los bytes del PDF viajan después. O sea que `await response.blob()`, la parte lenta de la ruta lenta, corría con el timer ya apagado y fuera de todo `catch` que pudiera traducir el abort: un backend que contesta headers rápido y después se traba seguía colgando para siempre, justo el bug que el cambio decía cerrar. La corrección acotada movió el `try/catch/finally` para que cubra el `fetch`, la rama de respuesta no-ok y la lectura del cuerpo, igual que hace `request()` | `frontend/src/services/__tests__/api.test.ts` — dos tests en el describe `timeout / abort`: «downloadBlob aborts past its own PDF timeout and reports it as a timeout, not a cancellation» (rojo antes con `expected undefined to be defined`, porque `fetch` nunca recibía `signal`) y, de la corrección, «downloadBlob still aborts when fetch resolves but the body stream stalls», que hace resolver el `fetch` y deja colgado el `blob()`: contra la primera versión del fix se colgaba en vez de rechazar con `TimeoutError`. Corridos el 9 de agosto de 2026: `cd frontend && npx vitest run src/services/__tests__/api.test.ts` → 52/52; suite completa `npx vitest run` → 161 archivos, 2441/2441; `npm run type-check` y `npm run lint` sin errores |
